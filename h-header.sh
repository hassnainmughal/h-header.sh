...
if [ -e $PREFIX/share/figlet/Mirza047.flf ]; then

	echo -e "\e[1;34m[*] \033[32mMirza047.flf figlet font is present\033[0m";

	sleep 4

else

wget https://raw.githubusercontent.com/mirza0047/Mirza047/master/Mirza047.flf &> /dev/null;

sleep 3

cp Mirza047.flf $PREFIX/share/figlet/Mirza047.flf;
...
bash ~/H-Header/.banner.sh ${col} ${HNAME}
...
if [[ ${PROC32} == [Y/y] ]]; then
	...
	if [ -e $HOME/h-header.txt ]; then

		rm $HOME/h-header.txt;

	fi

	if [ -d $HOME/H-Header ]; then
...
%{\e[0;31m%}┌─[%{\e[1;34m%}%B%{\${HNAME}%}%{\e[1;33m%}@%{\e[1;36m%}termux%b%{\e[0;31m%}]─[%{\e[0;32m%}%(4~|/%2~|%~)%{\e[0;31m%}]%b
...
echo -e "\033[31m$(figlet -f Mirza047 "H- Header")\e[0m"

echo -e "\e[1;32m

+----------------------------------*/

Mirza047 : (\e[33m14.4.2021\e[32m)
